 ...
 * ```
 */
const createKYCService = (apiClient: ReturnType<typeof createApiClient>) => {
  return {
    /**
    ...
    aadhaar: {
      ...
      generateOTP: (request: AadhaarOTPRequest): Promise<AadhaarOTPResponse> =>
        apiClient.post<AadhaarOTPResponse>('/api/v1/kyc/aadhaar/otp', request),
      ...
      verifyOTP: (request: AadhaarVerifyRequest): Promise<AadhaarVerifyResponse> =>
        apiClient.post<AadhaarVerifyResponse>('/api/v1/kyc/aadhaar/otp/verify', request),
    },
    ...
    status: {
      ...
      get: (userId: string): Promise<KYCStatus> =>
        apiClient.get<KYCStatus>(`/api/v1/kyc/status/${userId}`),
    },
  };
};
